...
This pipeline creates a local directory (i.e., the 'HT29R_resFolder') where the reference HT-29 data-set will be downloaded. A sub-directory will be created in the 'HT29R_resFolder' to save results' plots. Feel free to amend the below paths as required.

```{r }
dir.create('~/Documents/HT29R_resFolder/')
tmpDir <- path.expand('~/Documents/HT29R_resFolder/')
...
cguides <- intersect(cguides, Reduce("intersect", lapply(1:length(DF),
                                                         function(x){DF[[x]]$sgRNA})))
...
# uncomment if a .tsv file is desired:
# write.table(DF, file=paste(resultsDir,"User_data.tsv",sep=""), row.names=TRUE, sep="\t")  
...
```{r echo=FALSE}
HT29R.downloadRefData(destFolder = "~/Documents/HT29Data/",whatToDownload = "rawCounts" )
```
